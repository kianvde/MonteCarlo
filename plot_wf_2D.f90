SUBROUTINE metropolis_hastings(bucket)
    ...
    integer :: i, bi_x, bi_y
    real(8) :: p, p1, p2, PI, phi1, phi2, phi12, phi1n, phi2n, phi12n, r12, r12n
    ...
    DO i=1,5000000
        ...
        r1n = r1 + 0.01*sqrt(-2*log(rnd(1)))*cos(2*PI*rnd(2))* & 	
            (/ cos(2*PI*rnd(3)), sin(2*PI*rnd(3)) /)
        r2n = r2 + 0.01*sqrt(-2*log(rnd(4)))*cos(2*PI*rnd(5))* & 	
            (/ cos(2*PI*rnd(6)), sin(2*PI*rnd(6)) /)            
        
        r12 = sum((r1 - r2)**2)**0.5
        r12n = sum((r1n - r2n)**2)**0.5
        
        phi1 = exp(-(sum((r1+s))**2)**0.5) + exp(-(sum((r1-s))**2)**0.5)
        phi2 = exp(-(sum((r2+s))**2)**0.5) + exp(-(sum((r2-s))**2)**0.5)
        phi12 = exp(r12/(2+2*r12))
        phi1n = exp(-(sum((r1n+s))**2)**0.5) + exp(-(sum((r1n-s))**2)**0.5)
        phi2n = exp(-(sum((r2n+s))**2)**0.5) + exp(-(sum((r2n-s))**2)**0.5)
        phi12n = exp(r12n/(2+2*r12n))
        
        p = ((phi1*phi2*phi12)*(phi1n*phi2n*phi12n))**2
